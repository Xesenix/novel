...
export function pickAndDropObservable(dragulaService: DragulaService, selector: string) {
	...
	const dragCancel = dragulaService.cancel.pipe(filter(([container]) => container === selector), mapTo(null), tap(v => console.log('cancel drag')));
	...
	return dragStart.zip(dragEnd).pipe(
